...
export function EditBookModal({
  ...
  const getAllProductionTasks = () => ['Cover Design', 'Pengantar & Endors', 'Editing Naskah', 'Draf PK & Peta Buku', 'Layout', 'Desain Peta Buku', 'Print & Proofread Awal', 'QC Isi & ACC Kover Final', 'Finishing Produksi', 'SPH', 'PK Final', 'Cetak Awal Dummy', 'Proofread Akhir', 'Input Akhir', 'Cetak Dummy Digital Printing (opsional)', 'Naik Cetak', 'Turun Cetak'];
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField control={form.control} name="author" rules={{
              ...
              <FormField control={form.control} name="pic" rules={{
              required: 'PIC wajib dipilih'
            }} render={({
              field
            }) => <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="bg-background border z-50">
                        {teamMembers.map(member => <SelectItem key={member.id} value={member.name}>
              ...
              <FormField control={form.control} name="publisher" rules={{
              ...
              <FormField control={form.control} name="deadline" rules={{
              ...
              <FormField control={form.control} name="status" rules={{
            ...
            field
                  ...
                  <FormControl>
                    <Textarea placeholder="Masukkan catatan tambahan..." className="min-h-[80px]" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>} />
                    ...
                    {fields.map((field, index) => {
                      ...
                      return <TableRow key={field.id}>
                          <TableCell className="font-medium">
                            {editingTask === `${index}` ? <Input value={editingTaskData.name || ''} onChange={e => updateTaskField('name', e.target.value)} className="min-w-[150px]" /> : field.name}
                          </TableCell>
                          <TableCell>
                            {editingTask === `${index}` ? <Select value={editingTaskData.pic || ''} onValueChange={value => updateTaskField('pic', value)}>
                          ...
                          <TableCell>
                            {editingTask === `${index}` ? <Input type="date" value={editingTaskData.deadline || ''} onChange={e => updateTaskField('deadline', e.target.value)} className="min-w-[150px]" /> : task?.deadline ? new Date(task.deadline).toLocaleDateString('id-ID') : ''}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            {editingTask === `${index}` ? <Textarea value={editingTaskData.notes || ''} onChange={e => updateTaskField('notes', e.target.value)} className="min-h-[60px] min-w-[150px]" placeholder="Tambahkan catatan..." /> : <span className="text-sm">{task?.notes || '-'}</span>}
                          </TableCell>
                          <TableCell>
                            {editingTask === `${index}` ? <Select value={editingTaskData.status || ''} onValueChange={value => updateTaskField('status', value)}>
                                <SelectTrigger className="min-w-[120px]">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                              ...
                              </Select> : <Badge className={cn("text-xs", getStatusColor(task?.status || 'Not Started'))}>
                                {task?.status || 'Not Started'}
                              </Badge>}
                          </TableCell>
                          <TableCell>
                            {editingTask === `${index}` ? <div className="flex gap-1">
                                ...
                                <Button size="sm" variant="ghost" onClick={() => handleEditTask(task, index)} className="h-8 w-8 p-0">
